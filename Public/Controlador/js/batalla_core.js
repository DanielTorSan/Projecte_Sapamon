...
/**
 * Versión optimizada de la actualización periódica del estado de la batalla
 * Esta versión reduce la carga del servidor usando un timestamp para solo obtener cambios
 */
function iniciarActualizacionPeriodica() {
    let ultimaActualizacion = Date.now(); // Timestamp de la última actualización
    
    function actualizarEstado() {
        fetch(`../Controlador/batalla_api.php?action=estado_batalla&id_batalla=${batalla.id_batalla}&timestamp=${ultimaActualizacion}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                if (data.hay_cambios) {
                    ...
                    if (nuevaInfo.missatges && nuevaInfo.missatges.length > 0) {
                        nuevaInfo.missatges.forEach(missatge => {
        ...
        .catch(error => {
            console.error('Error:', error);
            // En caso de error, esperar un poco más antes de reintentar
