    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateUserAvatar(avatarName) {
                ...
                fetch('Controlador/Usuaris/update_avatar.php', {
                    method: 'POST',
                    body: formData
                })
            ...
            window.gestionaAmistat = function(sollicitudId, accio) {
                ...
                fetch('Controlador/Usuaris/Amics/gestiona_amistat.php', {
                    method: 'POST',
                    headers: {
            ...
            function loadChatMessages(friendId) {
                fetch(`Controlador/Usuaris/Chat/cargar_missatges.php?amic_id=${friendId}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
            ...
            function sendChatMessage() {
                ...
                fetch('Controlador/Usuaris/Chat/enviar_missatge.php', {
                    method: 'POST',
                    body: formData
                })
                .then(response => response.json())
            ...
            function checkNewMessages() {
                if (currentChatFriendId) {
                    let url = `Controlador/Usuaris/Chat/cargar_missatges.php?amic_id=${currentChatFriendId}`;
                    if (lastMessageTime) {
            ...
            function updateUnreadMessagesIndicators() {
                // Añadir verificación adicional para comprobar si estamos en la página de equipos
                try {
                    // Verificar que el usuario está autenticado antes de intentar comprobar mensajes
                    if (!<?php echo isset($_SESSION['usuari_id']) ? 'true' : 'false'; ?>) {
                        console.log('Usuario no autenticado, no se comprueban mensajes no leídos');
                        return;
                    }
                    ...
                    fetch('Controlador/comprovar_missatges.php')
                        ...
                        .catch(error => {
                            console.log('Error al comprobar mensajes no leídos:', error);
                            // No mostrar errores al usuario para no interrumpir la experiencia
                        });
                } catch (error) {
                    console.log('Error en updateUnreadMessagesIndicators:', error);
                }
            }
            ...
            window.sendFriendRequest = function(userId) {
                fetch('Controlador/Usuaris/Amics/friend_action.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
            ...
            function eliminarAmigo(amicId, friendElement) {
                ...
                fetch('Controlador/Usuaris/Amics/eliminar_amic.php', {
                    method: 'POST',
                    body: formData
                })
                .then(response => {
